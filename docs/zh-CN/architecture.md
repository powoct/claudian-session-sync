...
| **machineId** | 单台机器的本地标识（绝不同步），只用于显示与审计，**从不参与决策**（§10.3）。⚠️ 但它**换值**是有代价的：`parseObservations` 见到不同的 id 会判整本账本 `unusable`，所以 ADR-74 之后**只铸一次、永不重铸** |
...
| **equivalent** | 两侧**等长**、逐条记录相同、只差对象成员顺序（ADR-75）。字节上属于 divergent，语义上不是分歧；只对 append-jsonl、只在体积上限内计算 |
...
**丢失 / 损坏 / fingerprint 不匹配 → 视为全空**，所有条目的 `firstSeenMs` 重置为 now，于是本次 pass 中所有需要"稳定"前提的动作（全部 OVERWRITE、全部 CONFLICT 隔离、远端侧的 PULL_NEW —— **ADR-70 之后没有例外**）统统 `DEFER`，pass 退化为"只读 + 建立观察"（此时仍会在 P8 写回本文件，否则下一轮无从恢复），下一次恢复正常。这是**故意的 fail-safe**：丢 ledger 的代价是慢一轮，而不是误判。`lastSeenMs` 超过 30 天未出现的条目在 commit 时 GC。
...
| **Grok** | **Tier A/R 混合(逐文件分级),可写 ✅** —— 2026-08-24 双平台 P6 探测(grok 1.0.5,38 张生命周期快照,[findings](./findings/2026-08-24-grok-probe.md))回答了全部阻塞问题:**发现机制是扫目录**(`session_search.sqlite` 是搜索缓存,把会话拷进无索引的全新 GROK_HOME 仍被列出)⇒ 不是 Tier B;**跨 cwd 落位成立**(搬到另一 cwd 的项目目录后可见、可 resume、历史完整)⇒ 可跨机同步;**不变式 G1 成立,primary = `summary.json`**(唯一缺席即从列表消失、且不被 CLI 重建的文件)。group 成员**两张决策表并存**:`chat_history.jsonl`/`updates.jsonl`/`rewind_points.jsonl` 严格追加(**正常使用期间** PREFIX_VIOLATION 0;⚠️ **rewind 与 `/compact` 会截断前两者之外的 `chat_history.jsonl` 与 `rewind_points.jsonl`**,见 OQ-14),`summary.json` 每 turn 整份重写 ⇒ 走 §7.2b。**混龄 group 自愈**(回滚 summary + 最新 chat_history ⇒ resume 正常,summary 被按当前历史重新生成,但陈旧副本独有的状态会被静默丢弃)——这条实测是 §6.6 不为 Grok 引入 staging 事务的依据。⚠️ **缺 `summary.json` 时 resume 会把 turn 追加进另一个会话**(双平台同形),这是 §6.6.1 三条硬规则的由来。**2026-08-26 两机验收通过,`experimental` 已摘**(九步:G4/G7 跨机 resume 双向成立、G8 半落地零落地;G9 在面板缺陷 F-4 修复后真机复验通过 —— [findings](./findings/2026-08-26-grok-acceptance.md))。**默认关闭**(ADR-39,与 Tier 无关) | A/R(逐文件) | 无。未测项:无（OQ-16 已于 2026-08-30 关闭，ADR-64）。**OQ-14 已测(2026-08-29)并查出阻塞级缺陷:rewind 会被 `PULL_OVERWRITE` 静默撤销**,见该行 |
...
不回滚的理由：回滚 aux 本身是**一次新的破坏性写入**（在已经出错的状态下追加写操作是放大风险）；由 G1，孤立 aux 对 CLI 无害；rename 是幂等的，下次 pass 会重走完整流程。代价是本机可能残留孤立 aux——**清理命令已交付**(2026-08-30,ADR-60):命令面板的
「Clean up half-copied sessions」,**永远不自动删除**。

难点不在找,在于**孤立 aux 与「CLI 正在新建的会话」形状完全相同**——都是「有成员、没有
提交点」。所以候选要连过四关,后两关才是防止误删对话的那两关:

1. **provider 认得这些文件**——由 adapter 的 `listIncompleteSessions()` 回答,不在通用代码里
   走目录。`listSessions` 刻意丢弃没有 primary 的组(引擎不能组装它),而清理恰恰要看这些,
   所以这是一个**独立的、可选的**接口方法:布局知识留在 provider 里;
2. 这一组确实缺 primary;
3. **sync 目录里也没有它的提交点**——有的话下一轮 pass 会补齐,此时提议删除是在跟一次
   已经注定发生的修复赛跑。判据是列 replica 目录、把每个名字交回 `classifyNeutral`,
   **不猜** primary 叫什么(那是 provider 知识);
4. **这个会话安静够久**(10 分钟)——正在被写的会话是正在被新建的会话,不是残留。

删除前**先备份**(与每一次覆盖同一条规则,I1):这是全插件唯一以销毁字节为目的的命令,
备份不成则一个都不删。
...
| 5 | 任一侧 unstable（含 0 字节首次观察、末段不可解析）。**没有例外**（ADR-70 撤销了原 §9.1.3 快速通道那一条） | `DEFER` |
| 6 | 已知冲突且确定性隔离副本已存在（§8.1） | `NOOP`，并在 Action 上置 `conflictKnown: true` |
| 7 | 两侧 `contentHash` 相同 | `NOOP` |
| **7b** | 两侧**等长**、字节不同，但逐行规范化（成员按键名排序、标量**原样照抄**）后完全相同（ADR-75） | `PULL_OVERWRITE`（`equivalent-serialisation`）；若本机已按**同一份**远端字节收敛过一次，则 `NOOP`（`equivalent-left-as-is`） |
| 8 | 互不为字节前缀，且不是 7b | `CONFLICT` |
| **9a** | 一方是另一方的严格前缀,**但短的那侧已跌破本机见证过的收敛基点 size,而长的那侧仍含该基点**(OQ-14) | `CONFLICT`(`local-shrank-below-converged` / `remote-shrank-below-converged`) |
| 9b | 一方是另一方的严格前缀 | `PUSH_OVERWRITE` / `PULL_OVERWRITE` |
...
| 2 | 不存在 | 存在 | R stable | `PULL_NEW` | 对端 session 落地 |
| 3 | 内容相同 | — | — | `NOOP` | 已一致 |
...
**7b 为什么只向同步夹那一份收敛（ADR-75）**：两份都是同一段对话，没有哪一份更对，所以这里不是"选版本"而是"选一个吸收态"。**永不写进同步夹**的规则让同步夹那一份成为唯一的吸收态——任何一台都不会给对端制造新的变体，N 台机器各写一次本地即全体收敛。若改成"按 hash 大小选胜者"，约一半情况要 push，等于两次写、两份备份，还要往第三方同步工具正在写的目录里做原子写。**churn stop**：同一份远端字节只收敛一次；再次判 equivalent 说明是**本机**又被重新序列化了一遍（CLI 每次启动重写），这时把远端拉回来只会撤销它的改写、白白吃掉一个备份位，于是判 `NOOP` 放着不管——两份等价文件各自停在原地，不丢任何记录。

**7b 不肯原谅的**（每一条都会退回 `divergent`）：标量按源文本逐字比较（`JSON.parse` 会把 `{"n":12345678901234567890}` 和 `…891` 折成同一个 double，从而授权一次真正的覆盖）；重复键直接判本行失败（排序后两份成员表相同，last-wins 又让结果取决于到达顺序）；数组顺序；行边界移动（codex 的 paginated 历史按**字节偏移**建索引，"每条记录仍从原位开始"是对一个本插件并不拥有的文件许下的承诺——等长排列天然保证这一点）；任何解析不了的行。
...
短路顺序（都不读字节）：`size` 相同且 hash 相同 → `NOOP`；`size` 相同但 hash 不同 → 直接 `divergent`（相同长度不可能互为严格前缀）；否则才进 `isPrefix`。⚠️ ADR-75 之后，「等长且 hash 不同」这一格**不再直接进决策表**：append-jsonl 且在体积上限内时，引擎在调用点再问一次 `compareRecordwise`，只有它也说 `divergent` 才是 `divergent`。这道判断留在**调用点**而不是 `relate()` 里——`relate()` 是 `PrefixRelation` 的唯一产地，保持它只讲字节、不认 provider。
...
其他约束：workspace 未初始化时报 `WORKSPACE_NOT_INITIALIZED` 并**列出它将会创建什么**，而不是"顺便创建一下"；machineId 的**首次铸造**与**改名**在 dry-run 下只在内存生效，并在报告里说明少写了什么（ADR-76；ADR-74 之后已无「轮换」一说）；adapter 的 `reconcileLocalIndex()` 必须读 `ctx.dryRun` 并短路；**首次绑定一个新的 syncDir 时 `dryRun` 默认置 `true`**，强制用户看一眼计划再放行。
...
> **对多文件 provider,这个判定按 group 算,不按单文件算**(2026-08-27 的 OQ-17 实测,
> 2026-08-30 实装)。实测 Grok 在一次 turn 中途把 `chat_history.jsonl` 静置了 **23 秒**,
> 而同期 `events.jsonl` 走了约 90 个版本——**要同步的那个文件恰好在对话进行中看起来最像
> 已经停了**,一轮 pass 若在此时判它稳定,推上去的就是最终版本并不延伸的中间态。
>
> 判据:`SessionGroup.witnessPaths` 声明「哪些本机文件能说明这个会话此刻有没有人在写」,
> 引擎把它们合成**一个** E0 签名交给同一套 `judgeStability`,不稳定则整组 `DEFER`
> (reason `session-being-written`)。三条设计取舍:
>
> - **只 `lstat`,不读 tail**——活跃的会话每几百毫秒就变尺寸,读十来个文件的尾部什么也买不到;
>   逐文件那一层仍然带 tail hash,这是「带子」,那是「背带」。
> - **锁文件排除**:恒 0 字节且伴随进程整个生命周期,说明不了正在不正在写。
> - **`events.jsonl` 是最灵敏的信号,而它恰好是 `derived`、本来就不同步**——拿它做判据零传输成本。
>
> 会话是单文件的 provider 不声明 witnesses,行为一字未变。
> 详见 [findings/2026-08-27-oq17-grok-streaming.md](./findings/2026-08-27-oq17-grok-streaming.md)。
>
> ⚠️ **顺带记一条独立缺陷**:`probeDelayMs`(默认 400ms)从设置一路传到引擎、§9.1 的注释也写着
> 「两次观察间隔 probeDelayMs」,但**观测路径里没有任何 sleep**——两次 `lstat` 是背靠背取的。
> 所以「轮内再观察」这道防线目前不存在,真正在工作的只有跨轮比较 + 静默窗口。
> **把它实现出来也修不了 OQ-17**(400ms 在那 23 秒的平台期里同样看不到变化),但一个配了却
> 什么都不做的旋钮比没有更糟。按文件睡是 400ms × N(百会话即 40 秒一轮),要做得对得改成
> 「先扫一遍全部 o1 → 睡一次 → 再扫一遍 o2」,是一次 pass 循环重构。记为 **OQ-20**。
...
`futureMtime(f) := f.mtimeMs > nowLocal + clockSkewToleranceMs`。命中时**不**直接判 unstable（否则永久 DEFER），而是从签名中剔除 mtime 分量、只用 `(size, ctime, ino, tailHash)` 继续走 ledger 路径，报告标 `futureMtime`。默认（**以代码为准**，2026-08-30 核对）：`localQuietMs = 15000`（OQ-16 实测定值）、`remoteQuietMs = 8000`、`clockSkewToleranceMs = 5000`。`probeDelayMs` 已随 ADR-63 移除。
...
> ~~**PULL_NEW 快速通道**~~ ⛔ **已撤销（2026-09-01，ADR-70）**，代码与规范一并删除。
>
> 它承诺的是「对端新会话不必等满 `remoteQuietMs` 就落地」，四个条件里第三条是**「pass 内两次探测签名一致」**——那正是 [ADR-63](#adr) 废除的轮内第二次观察，所以接上它的第一步是把一个已被三路独立反驳的机制请回来。剩下的信号撑不住：`fullyParsed`（每行 `JSON.parse` 成功）**对 append-only 文件结构性失明**——半落地的前缀本身就是一份合法的 JSONL，正是它必须拦下的那种情况。
>
> 而「最坏后果只是多一个不完整的新文件」这句**低估了**：文件一落地，用户就能在本机 `resume` 这个会话并往这份前缀上追加；此后两侧互不为前缀 ⇒ **CONFLICT**，出现在一条用户刚开始用的会话上。这与 ADR-61 花了一个版本修掉的形态是同一类。
>
> 收益上限也只有**一轮自动 pass**（默认 5 分钟）、且只在会话第一次落地时；赶时间的用户点两次「Sync now」（间隔 ≥ `remoteQuietMs`）即可。删除是**行为等价**的：`pullNewFastPath` 在生产代码里一直硬编码为 `false`。
...
> | 外部同步工具丢失、回滚或损坏了 sync-dir 内容 | 远端内容退化 | §9.6 就绪状态机检测并停止写入。⚠️ **「本机文件不受影响」有一个已实测的例外**，见下一行 |
> | **同步工具在两个版本之间做了选择，把本机推上去的那份改名搁置**（Dropbox/Syncthing 的冲突副本） | 落败机器上,本机文件会被对端版本覆盖——本机看到的是「我没改、远端改了」,与对端正常编辑**在字节层面完全无法区分**(ADR-57 有证明) | 覆盖前必有备份;**且当同步工具留下了可读的副本时,本插件会认出那是自己的字节并改判 CONFLICT**(ADR-57,reason `opaque-push-set-aside-by-sync-tool`)。**留不下副本的传输方式(iCloud、部分 Syncthing 配置、裸共享盘)没有兜底**——只有备份区,以及报告里的一句汇总提示。**可恢复,但需要用户介入** |
>
...
interface PortableSettings {
  ...
  schemaVersion: 2;                    // 读到更高版本时原样保留，不降写（ADR-68）
  localQuietMs: number;                // 15000（OQ-16）
...
| 机器身份 | 两台机器共用 machineId 导致审计错乱 | §10.3；machineId 从不参与决策，最坏后果限于显示不准。**不再靠 hostname 漂移检测**（ADR-74：hostname 可手改、且 macOS 未设 `HostName` 时会随网络变），换成「陌生账本时出声」 |
...
| 22 | machineId 冲突时自动轮换、不打扰用户 | 弹窗让用户决定 | machineId 从不参与决策，轮换零风险；弹一个用户无法判断的窗只会造成困惑。⛔ **「轮换零风险」是错的,已被 ADR-74 推翻**:轮换会让 `parseObservations` 判 `machine-id-mismatch` ⇒ 整本观察账本作废 ⇒ 连 ADR-61 的收敛基点一起没了。实测一台机器一天轮换 9 次 |
...
| 57 | **OQ-18:#4b 增加一个「本机的推送是否被同步工具搁置」的判据**——在 replica 里找与本文件同名前缀的兄弟文件,**读它的字节**,若等于本机当前内容则改判 `CONFLICT`(reason `opaque-push-set-aside-by-sync-tool`);只作用于 #4b,不作用于 #4a;**不新增任何持久状态** | (a) 按「冲突副本是新出现的」判(我的初版) ／ (b) 给收敛基点打「来自本机推送」的标记 ／ (c) 记住上次推送的哈希,「remote ≠ 我推的」即判回滚 ／ (d) 发布 lineage 记录 | **先证明了「仅凭内容 + 本机 ledger」无解**:构造两条执行——E1「对端拿到我的版本后正常改」与 E2「我的推送被传输丢弃」——在落败机器上留下**完全相同**的 local 字节、replica 字节、ledger 基点与逐轮观察历史,而正确动作一个是 PULL 一个是 CONFLICT。这一刀同时否掉 (b)(c) 与所有基于 mtime/inode/回退检测的廉价修法;(b) 另有独立死因:推送后的第一轮就会观察到 local == remote,把「推送来的基点」洗成「观察到的基点」。(a) **被三条独立理由杀死**,最要命的一条是时序:同步工具替换 replica 文件的那一轮,该文件的 E0 签名恰好变了 ⇒ 判 `DEFER · side-unstable`,**做决定的不是那一轮**;于是副本在「不决定的那轮」是新的、在「决定的那轮」是旧的。它还会被 SKIPPED_BUDGET、NOT_READY(正是同步客户端刚重启时的状态)、healthCheck 失败等空转轮消耗掉。**改用内容比对**则同时解决时序与误报:#4b 里 `local == base`,所以「副本装着我的字节」就是**正面证据**——在赢家机器上必然为假(它的 local 是新版),在第三台空闲机器上必然为假(它的 local 是分叉前的内容),手工复制或改名撞车也不会命中,而且本机记录一被重写就自动失效。候选来自**插入关系**而不是 `classifyExternalArtifact` 的置信度阶梯:唯一真机见过的副本是中文 Dropbox 的 `(… 的冲突副本 2026-08-29)`,它落在 medium 一档,按 `high` 卡会**恰好漏掉这个方案存在的理由**。**不碰 #4a** 有证明:推送本身设置了基点,所以被丢弃后落败方必然落在 #4b 或 #4c;而副本同样会传到赢家那台,在那里取证会让它每改一次就冲突一次——那正是 claudian 验收唯一的硬阻塞项 C4 与 Grok 的 G5。判据只被允许**收回一次写入**,永远不能授权一次写入,所以 EV-1 与 §5.6 规则 3 字面成立,改动按构造比现状更安全。(d) 覆盖面更好且与传输无关,但**两台都升级前谁也保护不到**,故列为后续 |
| 58 | **多文件 provider 的稳定性判定改为按 group 算**:`SessionGroup.witnessPaths` 声明本机的「活跃度见证」文件(只 `lstat`、从不读写、从不同步),合成一个 E0 签名走同一套 `judgeStability`;不稳定则整组 `DEFER`(`session-being-written`) | (a) 维持逐文件判定 ／ (b) 把 `localQuietMs` 调大 ／ (c) 按会话目录的 mtime 判 | (a) 被实测否掉:Grok 在一次 turn 中途把 `chat_history.jsonl` 静置了 **23 秒**,而同期 `events.jsonl` 走了约 90 个版本——**成员之间的静默是错开的**,要同步的那个文件恰好在对话进行中看起来最像停了。(b) 治不了本:平台期有多长,窗口就要多长,而那是模型出字的速度决定的、没有上界;调大还会拖慢所有 provider。(c) 不可靠:追加写不改目录 mtime。**用一组文件的合成签名**则既复用了既有的判定机器(连同它对未来时间戳、时钟回拨、丢失 ledger 的处理),又把「这个会话此刻有没有人在写」问成了一个可观察的问题。只 `lstat` 不读 tail:活跃会话每几百毫秒就变尺寸,读尾部买不到东西,而逐文件那层仍带 tail hash。会话是单文件的 provider 不声明 witnesses,行为一字未变——爆炸半径就是 Grok 一个 |
| 59 | **`WorkspaceBinding` 增加 `vaultPath`,binding 按「当前打开的 vault」选**,而不是 `listBoundWorkspaces()[0]` | (a) 维持现状 ／ (b) 一台机器只允许一个 vault(在 UI 里说明) ／ (c) 用 syncDirPath 反推 | 现状只在「一台机器一个 vault」时正确。两个 vault 时它取排序第一个,于是设置面板显示**另一个 workspace** 的 sync 目录与 provider 开关、Folder path 禁用,而身份比对拿本 vault 的 id 去比那个 workspace ⇒ 判 `WORKSPACE_IDENTITY_CHANGED` ⇒ 停同步。**这是 ADR-21 的 fail-closed 护栏被错误触发**,不是它在正常工作;2026-08-29 的验收被它挡住,操作者临时把真实 binding 移出 `workspaces/` 才跑下去。(b) 是把缺陷写成规格,而多 vault 是 Obsidian 的常态。(c) 不成立:两个 vault 完全可以指向同一个 sync 目录的不同 workspace 子树(§5.1 的布局就是为此设计的),而且用户随时可以改 syncDirPath。**加一个字段**则让「这个 binding 属于哪个 vault」从「无从判断」变成「记着」,护栏因此**更准**——比对对象从「本机随便哪个 workspace」收紧为「这个 vault 的那个」。迁移刻意保守:旧 binding 首次打开自己的 vault 时补盖,**补盖完成前保持旧行为**,因为未打戳的 binding 有可能就是本 vault 的,此时「没人认领」还不能读成「这是新 vault」——那会让身份被替换的 vault 收到一个「创建身份」按钮,正是 ADR-20 要防的分裂 |
...
| 64 | **`localQuietMs` 默认 3000 → 15000**,依据是把 OQ-17 的 100 ms 采样重读一遍算出的**合并静止时长** | (a) 维持 3000 ／ (b) 取 8000 ／ (c) 不设固定窗口,改为等到「所有见证文件都静止 N 个采样周期」 | **(a) 被实测否掉**:把两轮 turn 里所有非 lock 文件的变化时刻合并——正是 ADR-58 的 group 见证会看到的那个合成签名——最长整组静止是 **3,344 ms 与 8,212 ms**,3000 ms 的窗口在一次 turn 里被骗 1–2 次。**最有说服力的一行是 `events.jsonl`**:全场最忙(23 秒 96 个版本、中位间隔 101 ms)、也是见证里最活跃的那个,**连它都停了 8.2 秒**——所以合并静止不是被懒文件拖出来的,是会话真的停了。ADR-58 修对了粒度,但没定值,这条补上。(b) 只清掉 run 1。(c) 是把窗口换个说法,平台期没有上界这一点不变,还多一套状态。**代价近零**:默认 5 分钟的 pass 间隔下窗口从不生效(下一轮无论如何 300 秒后);只有「turn 刚结束 15 秒内点 Sync now」会多 defer 一轮,而那正是该 defer 的时刻。**不保证**:§9.1.5 与 ADR-58 的话仍然成立——平台期由模型出字速度决定、没有上界,两轮样本也只是两轮;这个数字只买到「已实测到的平台期塞不进窗口」。`remoteQuietMs` 未动(是另一种现象,无数据) |
| 65 | **准入扫描下探 `devices/device-<64hex>/`(Claudian 2.2.5),且读取所有设备目录而不只是本机那个** | (a) 维持单层 ／ (b) 无限递归 ／ (c) 照抄上游、只读本机设备目录 ／ (d) 只认设备层,把顶层当过渡态 | **(a) 就是这次的缺陷**:2.2.5 起新建会话的 `.meta.json` 落在 `devices/<deviceKey>/`,单层 `listDir` + `isFile` 把整个目录丢掉 ⇒ 会话不进候选集 ⇒ **不产生 report 行、不弹 Notice、状态栏照常 up to date**,四个 provider 全中(共用 `readVaultScope`)。(b) 违反 §8.2 fail-closed:store 里用户可能放别的东西。**两层 + 按上游 `InstallationKey` 的 `device-[a-f0-9]{64}` 校验目录名**是最小且够用的。**(c) 是最需要解释的否决**:上游的列表**确实**按设备作用域(`selectSessionMetadataCandidate` 见到 `assignment.deviceKey !== deviceKey` 直接返回 null,「Assign to this device」按钮即为此),但准入回答的是另一个问题——**这个 vault 认不认识这个 session id**。按设备作用域意味着 Mac 上新建的会话**永远无法把字节搬到 Windows**,而那是本插件存在的唯一理由。**准入放行,显示与否仍由 Claudian 决定。**(d) 不成立:上游自己仍按 device → unscoped → legacy 三级回退,发现该问题的 vault 里顶层还有 111 条存量记录——这不是过渡态,是分层的 store。**墓碑按层配对**,照抄上游而非近似:`deviceDeleted` 只压设备层、`unscopedDeleted` 压顶层与 legacy;跨层配对会让一台机器的删除隐藏另一台仍持有的会话。三条各有一个会红的注入。✅ **2026-09-01 真机复验通过**(验收步骤 A,[findings](./findings/2026-09-01-claudian-2.2.5-acceptance.md)):记录只在设备层时,它的会话文件照样进 replica——「准入放行,显示与否由 Claudian 决定」在真机上成立 |
| 66 | **`healthCheck()` 增加 `warnings`,并让准入扫描报出「store 里没读的目录」**(OQ-26) | (a) 只在 `storeFound && sessionIds.size === 0` 时告警 ／ (b) 挂在 `SessionGroup.unprovenOmissions`(ADR-62)上 ／ (c) 什么都不做,靠下次探测发现 | **通道的约束决定了方案**:2026-09-01 的失效形态是**零 group**——会话没被准入就不产生 group,没有 group 就没有 action,没有 action 就没有报告行,于是一次什么都没做的 pass 报「up to date」。所以 (b) 直接不成立:`unprovenOmissions` 挂在 group 上,恰好在最需要它的时候没有载体。`healthCheck()` 则**在 P1 每个 provider 跑一次、在任何 group 之前**,是唯一天然幸存的通道,代价只是给返回值加一个可选字段。(a) 噪音与漏报都差:一个 vault 里某个 provider 没有会话是常态(会天天响),而这次真实情形是**有 13 条被准入、只漏了新的那些**(`size === 0` 根本不成立)。**真正的信号是「store 里有我不下探的目录」**——正是这次的形状,而且正常情况下恒为空。(c) 是把「用户发现某个会话没过去」当作发现机制,这次已经证明它有多贵。四个 adapter 读同一个 store,所以引擎按文案去重——那是关于 vault 的一个事实,不是关于 provider 的四个。四条各有一个会红的注入 |
| 67 | **新增「把本机会话发布到 flat 层」开关(默认关闭)**,让 2.2.5 之后新建的对话能在其他设备的 Claudian 里**被看见**;镜像只写本机自己拥有的记录,且只在「仍是本机写的那份」时刷新 | (a) 复制进对端自己的 device 目录 ／ (b) 把本机记录从 device 层「降级」搬到 flat ／ (c) 什么都不做,只写进 README ／ (d) 无条件刷新镜像 | **问题的形状**:2.2.5 起新对话的记录落在创建它的设备目录下,而设备 key 是 `sha256(localStorage 种子)`、且 `SessionStorage` 只读**自己那个**目录——所以记录**无论怎么同步都过不去**,插件把字节搬过去了,对端侧栏里那一条却不存在。上游的取舍顺序是「自己的 device → flat → legacy」,**flat 是唯一存在的跨设备通道**,那正是它为「不属于任何设备的记录」保留的层。**(a) 制造两个所有者**:两台机器各有一份 device 记录、各自认为拥有、各自写——而它们是**不同路径的两个文件**,引擎压根不会判冲突,标题静默分叉。这是本项目整套设计唯一没有答案的形态。(b) 语义最干净(全局唯一一份),但 `create()` 对每条新会话都写 device 层,插件要**每轮把新会话反复降级**,等于跟上游默认行为长期对抗。(c) 放弃的是插件的目的本身。(d) 会静默覆盖对端的重命名——正是本轮刚修完 OQ-14 的那类错误。**代价必须写在设置文案里**:①拥有方仍读自己的副本,所以对端改的元数据**回不来**;②对端那条会显示「Assign to this device」按钮,一旦点下去会写 marker,而 marker 会把这条在原机彻底黑掉。`.assigned.json` **既不写也不复制**——它的检查在 `selectSessionMetadataCandidate` 里排在任何 metadata 路径之前(`SessionStorage.ts:71`),复制一份过去正好把本功能要暴露的会话全部黑掉。记账用 binding 里的 `mirroredConversations`(conv → 本机上次写的 hash),就是 ADR-48 收敛基点的本地版:没有它,只能在「永远不修正陈旧标题」和「静默覆盖别人的重命名」之间二选一。四条各有一个会红的注入,其中设备 key 必须按上游的 `device-[a-f0-9]{64}` 校验——它会成为路径段,不校验就能把 vault 外的文件读进来发布 ⚠️ **已被 ADR-69 取代**(独立审核推翻:flat 不是只读投影,复制会造出双可写权威,且删除后会复活) |
| 68 | **设置的 schema 升到 2,并把「曾是本插件默认值、现已实测为不安全」的取值迁一次**(目前只有 `localQuietMs: 3000 → 15000`) | (a) 什么都不做,新默认只惠及全新安装 ／ (b) 无条件把 3000 改成 15000 ／ (c) 改成在设置面板里提示用户自己调 | **(a) 等于 ADR-64 白做**:`data.json` 分不清「用户选了 3000」和「3000 曾是我们的默认值、被我们自己序列化进去了」——因为每次保存都会写全部字段。所以对默认值的实测订正**只能到达全新安装**,而跑得最久、对话最多的机器恰恰一条都拿不到。2026-09-01 的验收套件评审在准备 0.3.0 时发现了这一点。(b) 会改写真的手动选过 3000 的人的选择。(c) 把一个安全值变成一道选择题,而 OQ-16 的实测结论是它**不是**偏好:合并会话内全部非 lock 文件后,实录的两轮 turn 里整组静止 3.3 秒与 8.2 秒,3000 会把出字中途读成已稳定。**判据是三个合取项**:该键在 `SUPERSEDED_DEFAULTS` 里、`schemaVersion` 低于当前、且存着的值**恰好等于旧默认**。三条各有一个会红的注入。表里只有一条是刻意的:改写用户的设置是一次性的、有实测理由的动作,不是习惯。**2026-09-01 独立审核后补正两处**:(1) 原提交说「手动选过 3000 的人保持不变」是**错的**——schema 1 的文件根本区分不了「用户选的 3000」和「我们写进去的默认 3000」,两个承诺不可能同时兑现;这里选安全侧,代价写在注释与设置文案里,而不是假装没有。(2) `parseSettings` 原本无条件写回当前 schema,读到更高版本会**降写**,等于把未来版本的迁移权威抹掉、日后可能重跑迁移;现在高于当前版本的 schemaVersion 原样保留 |
| 69 | **把「让对端看得见」从「复制记录」改成「搬运记录」**:开关打开时,本机 device 层的会话记录**移入** flat 层(写 flat → 删 device,两阶段可恢复);开关改为**机器本地**(binding,不随 vault 同步);整个动作放进 pass 的锁内。**取代 ADR-67** | (a) 维持 ADR-67 的复制 ／ (b) 复制 + 字段级白名单投影 ／ (c) 复制进对端自己的 device 目录 ／ (d) 什么都不做,等上游给只读投影 | **(a) 被独立审核推翻,而复核确认它说得对**:flat 层**不是只读投影**,是上游正常的可写权威(`assertMetadataWriteAuthority` 在无 fence 时直接放行)。所以复制 = 一条会话有**两个可写权威、两个路径**,永不收敛,而引擎连它们是同一回事都看不出来(不同文件)。更实的是删除:上游删除的稳定态是 `markDeleted` 之后 `deleteCurrentMetadata` **删掉 device 的 meta**,只剩墓碑;而 ADR-67 的墓碑分支嵌在 `.meta.json` 循环里,metadata 一没就走不到 ⇒ 镜像永不移除 ⇒ 取舍链回退到镜像 ⇒ **用户删掉的会话复活**(当时那条测试保留了 metadata,是假绿)。**搬运把这些一次性消解**:只有一份记录 ⇒ 无双权威;`adopt` 把 flat 来源标成 `unscoped` ⇒ 每台机器都读得到、也都写回同一份;删除变成 `deleteCurrentMetadata(id,'unscoped')` + 同层墓碑,是真删除,没有影子可复活;「这份是不是我写的」这个问题连同它的 binding 记账一起消失。(b) 字段级投影更安全,但那是**造一份上游不认识的第三态**,而且仍是两个权威。(c) 更糟:两台各写各的 device 目录。(d) 作者已回复「it's feasible to make it works as intended」,没有给受支持的机制,也没有理由为本插件适配。**两阶段可恢复,不需要记账**:flat 不存在 ⇒ 写 flat 再删 device;flat 存在且字节相同 ⇒ 是上次中断的搬运,补完;**flat 存在且不同 ⇒ 不动并报告** —— 那是别人的记录或真分歧,不是本机可以覆盖的。**仍然承担的代价,写在设置文案里**:①关掉只停止搬新的,**不撤回已经共享的**(搬回去是第二次破坏性写入,而对端可能正在用);②共享出去的记录带着它的 `externalContextPaths`,那是绝对路径、在对端会变成 agent 的 `additionalDirectories` —— 但这与 2.2.5 之前所有记录都在 flat 层时**完全同形**,不是本插件伪造的新暴露面。**源侧防护**(审核 B-7,复现过):设备目录与记录文件都先 `lstat`,symlink 与 `nlink > 1` 一律拒绝。五条各有一个会红的注入。⚠️ **其中一条前提已被证伪,见 ADR-71**:「只有一份记录」只在下一次 Obsidian 重启前成立——上游每个会话只推导一次写入层,搬运之后它会按陈旧目标重建 device 副本。搬运本身没错,但它是**每轮的对账**而不是一次性迁移。✅ **2026-09-01 真机复验通过**(验收步骤 D,[findings](./findings/2026-09-01-claudian-2.2.5-acceptance.md)):flat +2 / 本机设备层 −2 **且只动本机那个目录**,一轮搬完;对端改名**回传到本机**(复制方案做不到);关掉不撤回、新会话不再搬;同意位落在 binding(`workspaces/<ws>.json` 489 → 520 B),`data.json` 未变 |
| 70 | **删掉 §9.1.3 的 `PULL_NEW` 快速通道**(OQ-22):`allowsPullNewFastPath()`、`PlanInput.pullNewFastPath`、`pullNewFastPath` 标志、以及规范里那一段一并移除;稳定性判定**不再有任何例外** | (a) 按规范接上 ／ (b) 换一个「远端确实写完了」的信号重做 ／ (c) 留着死码只改注释 ／ (d) 删掉并改规范 | **(a) 的第一步是复活 ADR-63 刚废掉的机制**:规范的四个条件里,第三条「pass 内两次探测签名一致」就是轮内第二次观察,而 ADR-63 已用三路独立反驳证明它没有独有的保护、且从来没生效过(`probeDelayMs` 一路传进 `EngineDeps` 却无人读)。**剩下的信号撑不住**:`fullyParsed` 对 append-only 文件**结构性失明**——半落地的前缀本身就是一份合法 JSONL,而那正是它必须拦下的情况;对 Grok 这种 group(`summary.json` 不是 JSONL)更是完全不适用,而 ADR-58 的 group 见证存在的理由就是不让一个成员先落地。**原规范的风险评估低估了后果**:它写「最坏是本机多一个不完整的新文件,下次 pass 补齐,本机原本没有任何东西可丢」——但文件一落地,用户就能 `resume` 这个会话并往前缀上追加,此后两侧互不为前缀 ⇒ **CONFLICT**,出现在一条刚开始用的会话上,与 ADR-61 花一个版本修掉的是同一类形态。(b) 要造的「远端写完了」信号就是静默窗口本身,没有更便宜的可信版本。(c) 是把一条做不到的承诺留在规范里。**收益上限只有一轮自动 pass(默认 5 分钟)、且只在会话第一次落地时**,赶时间点两次「Sync now」即可。**删除是行为等价的**:`pullNewFastPath` 生产代码里一直硬编码 `false`,所以这次改动不改变任何一条现有路径的判定——穷举矩阵从 41,472 降到 20,736 也正因为那一维本来就只有 `false` 有效 |
| 71 | **把「搬运」从一次性迁移改成每轮的对账**:机器本地记 `publishedHash`(本机**自己写进** flat 层的字节),据此判 FOLD(flat 仍等于基点 ⇒ 用 device 那份快进)/ RETIRE(device 仍等于基点而 flat 变了 ⇒ 对端接管,撤回本机副本)/ HOLD(都动过或无基点 ⇒ 留给人);device 层墓碑在 flat 仍等于基点时补写到 flat;共享步骤移进 `runPass` 的锁内 | (a) 维持 ADR-69 的「不同就 heldBack」 ／ (b) 用记录内的 `lastActivityAt`/mtime/超集关系判新旧 ／ (c) 无条件用 device 那份覆盖 flat ／ (d) 基点 + 双向退出 | **ADR-69 的前提错了,而错得不明显**:它假定搬完记录就留在 flat 层。实测(2026-09-04)上游把「往哪层写」缓存在 `ConversationRepository.metadataTargets`,**每个 Obsidian 会话只推导一次**(layout-ready 后的单次延迟扫描),没有任何 watcher 看 `.claudian/sessions/`,已缓存的会话**永不重读**。我们的搬运跑在同进程的 pass 里,**必然晚于**那次推导 ⇒ 映射**结构性地陈旧**,不是竞态;下一次元数据写入(聊一轮、改名、置顶、记 usage)就按陈旧目标重建 device 副本,而上游读取优先 device ⇒ **flat 那份从此冻结**。报告机器上 6 条,最严重的一条 flat 停在 97 KB 而 device 已 217 KB,对端落后三天。(a) 就是这个缺陷:「不同就 heldBack」把**本机自己上一轮搬过去的**当成别人的。(b) **三个信号全部被证伪**:`lastActivityAt` 是上游自己维护的活动状态、可回退;mtime 被 git checkout 刷新(报告里 9/4 00:17 那一批就是);「超集」对整份重写的 JSON 无意义。(c) 正是 ADR-67(d) 已被否决过的「无条件刷新镜像 ⇒ 静默覆盖对端的重命名」。**(d) 就是 §7.2b/ADR-48 的收敛基点,换个场景**:基点**只在本机写完 flat 之后设**,绝不从「在 flat 层看到的记录」种下——那一行看着无害,却是陷阱:上游永不重读,device 副本是从**早于对端编辑**的内存对象序列化出来的,把对端字节认作基点会让下一次快进覆盖掉对端的改名。**RETIRE 是唯一真正终结循环的一条**:device 副本等于基点说明它不含 flat 没有的东西,而它的存在正是上游忽略 flat 的原因,删掉之后下个 Obsidian 会话就把该记录认作 `unscoped`,此后本机一直写 flat。**顺带修掉一个已随 0.3.0 出厂的复活缺陷**:同一份陈旧映射让 `markDeleted` 把墓碑写进 device 层、`cleanupDeletedConversation` 删掉 device 元数据,flat 那份没有墓碑 ⇒ 下次扫描取舍阶梯落到 flat ⇒ **用户删掉的会话复活**——正是 ADR-67 被杀的那个失败,从另一扇门进来,而且是**我们的搬运造成的**,所以补写墓碑是修自己的破坏,不是传播别人的删除(ADR-10 管的是会话文件)。**同轮修掉三处安全欠账**:①共享步骤此前跑在**锁外**,而 `pass-runner.ts` 与 `plugin-runtime.ts` 两处注释都声称在锁内——本 ADR 给它加了破坏性写入,注释必须变成真的;②设备目录只 `lstat` 了叶子,`devices` 这一级被软链就能把 vault 外的记录读进来发布,改走 `resolveUnderRoot` 逐段;③`readDir(store).catch(() => [])` 把读不到的 store 当空目录,围栏与墓碑一起隐形 ⇒ 直落发布行 ⇒ 复活,改为**失败即中止**。发布改用 `writeFileNoReplace`,`target-exists` 是重排不是覆盖许可;每次覆盖前备份(拿不到备份就取消写),删除 device 副本前重新哈希复核(那个窗口正是用户在打字)。**留给人的那部分**:无基点或两侧都动过 ⇒ HOLD,并进「Repair shared conversation records」屏,列出两侧字节与时间,**一次一条**——每条是独立分叉,对一条的判断不带任何关于下一条的信息,所以没有「全部发布」按钮。**五条注入各自会红**,其中「store 读不到」与「删除前复核」两条起初全绿(与 `readStillValid` 同一形状),补了鉴别性用例才红:前者需要 `0o311`(可写可穿越、不可读),用 `0o111` 会因写也失败而假绿。⚠️ **2026-09-04 单机验收(§9.9)抓出两条阻塞级缺陷,均已修**:①**「看一眼就等于搬了」** —— 修复屏的 `sharingConflicts()` 跑的是完整对账**并写盘**,且**不看共享开关**(pass 路径有闸门,这条没有)⇒ 开关关着的用户只要打开一次那个屏幕,本机记录就全被搬进 flat。那个开关就是机器本地的**同意**,而打开一个屏幕不是同意;已加 `inspectOnly`,读取路径一字节不写、**也不落账本**(写入一个它没创建过的基点,正是本设计禁止的那件事)。②**「点一条却动全部」** —— `publishSharedRecord(id)` 只把 id 放进 `forced`,整轮对账照跑 ⇒ 用户被展示一条、点一个按钮,其余所有 device 记录也一并被搬;已加 `only` 把整轮限制在被点的那条。顺带把该按钮的写入放进 `withWriteGate` —— 它是 pass 之外的破坏性写入,而冲突解决/备份恢复/清理半份早就都走这道闸门。两条各有一个会红的注入 |
| 72 | **冲突面板的「已结束」判据从 `superseded` 布尔改为四值 `standing`**(`in-dispute` / `settled` / `moved-on` / `unreadable`),`settled` 写成**「某一支归档同时在两侧活文件上」**;按钮按 `settled` 置灰;`resolveConflict` 增加 `sides-agree` 拒绝 | (a) 维持原判据 ／ (b) `superseded ‖= (localHash === remoteHash)` ／ (c) 只改判据不动按钮 ／ (d) 只在动作层拒绝、面板不动 ／ (e) 往隔离目录里写 `resolvedAt` ／ (f) 把已结束的条目从列表里滤掉 | **原判据把最常见的结局判反了**:`!branches.some(b => b.onThisMachine ‖ b.inSyncFolder)` 要求两支归档都不再等于任何一侧活文件;而**解决一次冲突恰恰是让两侧收敛**,收敛后的字节按定义就等于其中一支 ⇒ 该支命中活文件 ⇒ 永远判「未解决」。**「已收敛」正是让它出不了列的条件。** 2026-09-09 实测:47 个目录两侧已逐字节相同,面板全部照给按钮,而状态栏(按 pass 判定计数)正确显示 1。点下去 `resolveConflict` 会**照常备份、轮转、把相同字节再写一遍并报成功**,徒增备份与一次 mtime 变更。**(b) 是最诱人也最危险的写法**:`hashOf` 把「文件不存在」「读不到(`kept-unreadable` 存在的理由)」「本机没启用该 provider(`localPathFor` 先返回 null)」「rel 在本机解析不出来」压成同一个 null,而最坏情况下**两个 null 同源**(schema-2 目录在嵌套 provider 下重建出的路径谁也指不到)⇒ `null === null` 会宣告两个**根本找不到的文件**达成一致 —— 那正是本次要修的方向的反面,且已被 `conflict-shape.test.ts` 作为既有缺陷钉住。它还会把云端脱水成 0 字节的两侧当成收敛(`hash(空) === hash(空)` 是真实且相等的非 null 哈希)。**走分支标志则两者都不成立**:`branch.hash` 恒为非空字符串,读不到的一侧配不上任何一支。**(c) 严格劣于现状**:收敛条目恰好有一支两个标志皆真,只改判据会在「无可选择」的句子正上方留下两个可点按钮。**(d) 不够**:面板与调色板命令读同一份列表且不读任何标志,但只改动作层则界面仍在撒谎。**(e) 被对端推回自己那支杀死** —— 目录是共享的,判定是**每机且可逆**的,写进去的标记会压掉同一 conflictId 下真实的复发。**(f) 被 `main.ts` 的 0/1/many 分支杀死**:滤掉会让它宣布「Nothing is in conflict.」而 62 个目录还在,且违背「归档永不删除、始终可 reveal」的承诺 —— **要标记,不要过滤**。**`settled` 证明了什么要写清**,这是置灰按钮的全部依据:某支两侧皆在 ⇒ `localHash === remoteHash === 该支哈希`,而 `resolveConflict` 是把从保留侧读到的字节写到另一侧 —— 那一侧已经是这些字节 ⇒ **此状态下任何按钮都改不了一个字节**,置灰不减少任何补救手段。撕裂读(local 在 t1、remote 在 t2)要造出假 `settled`,只能靠 t1 之后 CLI 本地追加,那落在「一侧已前移」的形状上,keep-local 本就会以 `branch-moved` 拒绝、keep-remote 则是把更长的文件截短 —— **无害,而不是不可能**。**`unreadable` 与 `moved-on` 必须分开**:一句话同时覆盖「两侧都读到了、都已前移」和「两侧都读不到」,后者正是旧代码对着磁盘上真实分叉说「已经结束」的地方。五条注入各自会红。顺带修掉 `extensionOf` 取 `readDir` 首个文件的扩展名 —— 未排序,而 Finder 恰恰在用户点「Show me both」时往该目录写 `.DS_Store`,`extensionOf('.DS_Store')` 返回 `.DS_Store`,会把一条真冲突推进 unreadable 桶;改为只认引擎写过的三种前缀(`branch-` 与历史的 `local-`/`remote-`,后者有回归测试保护) |
| 73 | **Codex rollout 按「线程 id 准入、rollout id 标识」两个 id 分开用**,白名单接受 `rollout-<ts>-<threadId>_<rolloutId>.jsonl`;备份路径超长时**丢掉 `<logicalId>` 那一层**,连扁平也放不下则**返回 null 取消覆盖而不是抛** | (a) 维持原状 ／ (b) 只把正则放宽、id 仍取第一个 uuid ／ (c) logicalId 用 `<threadId>_<rolloutId>` 复合 ／ (d) 一个线程一个 group、多文件作成员 ／ (e) 不搬运,只报告 | **(a) 是正在丢数据的现状**:上游 `rollout_file_name.rs` 的 `render()` 在 `rollout_id != thread_id` 时改名带 `_`,而 revert 恰好产生这种情况(`revert_thread.rs` 原文「Old rollouts stay intact」,只挪 SQLite 指针)。旧白名单没有 `_` 分支 ⇒ 对**当前那份**返回 null ⇒ `listSessions` 跳过、`classifyNeutral` 拒绝 ⇒ **一条 rewind 过的会话,它的最新历史从来没同步过,而 rewind 前那份照常同步、报告照常 up to date** —— 与 2026-09-01 `devices/` 同一种沉默形状。该形态 2026-08-12(#38127)就合入,用户跑的 0.153.4 里就有。**(b) 会把两个文件压到同一个 logicalId 上**,而 group 活跃度账本的键是 `<provider>/<logicalId>`(sync-engine.ts:404):两个 group 每轮互相覆盖对方的复合签名 ⇒ 双双 `changed-since-last-pass` ⇒ **永远 DEFER**。子代理在真实引擎上跑通了这个复现。**(c) 被 Windows 路径预算杀死**:logicalId 是备份目录里唯一的路径段(backup-store.ts),而实测最深路径已是 **239/259**,复合 id 再加 37 字符直接越界。**(d) 语义最漂亮但代价大**:成员分布在不同 `YYYY/MM/DD` 目录、且「primary 不在 replica 就整组 DEFER」会在原始文件本机缺失时变成**永久停滞而状态栏仍显示 up to date** —— 用一个沉默换掉另一个沉默。**(e) 不搬运则对端永远拿不到最新历史**,而这正是本插件存在的理由。**选中的方案关键在两个 id 用途不同**:准入回答「这个 vault 认不认识这条对话」——那是关于**线程**的问题,而 Claudian 只记线程 id(它自己也找不到这个文件,`CodexHistoryStore.ts` 用 `endsWith("-<threadId>.jsonl")`,同一个盲点);logicalId 是本插件的**每文件**身份,取 rollout id 才唯一。**未 revert 的线程两个 id 是同一个字符串,所以磁盘上现有的一切身份不变**:备份目录不搬、冲突 id 不变。**顺带修掉一个会掀掉整轮 pass 的隐患**:备份叶子名是源文件 basename,`_<rolloutId>` 让它再长 37 字符;ADR-54 写着「真超了 PathGuard 会拒绝」,但 `mintStatePath` 只查 NUL 与 containment、**根本不看长度**,于是超长一路走到 `mkdirp` 抛 ENAMETOOLONG,而 `deps.backup(...)` 在引擎 try 块**之外**、`runPass` 只有 try/finally ⇒ **所有 provider 的这一轮一起死**。修法是 `backup()` 整体 try/catch、返回 `backup-failed:<errno>`。**中途试过「先算预算、超长回落扁平」,已推翻**:扁平会把 ADR-54 消除掉的歧义放回来(共享目录下轮转按文件名圈定候选,Grok 的成员名跨会话重复),而且预测长度是在猜平台,猜低了会拒掉本可写入的备份 ⇒ 取消覆盖 ⇒ 静默停同步。**让文件系统自己回答**。四条注入各自会红,其中「准入用哪个 id」起初全绿(与 `readStillValid` 同形),补了走 `listSessions` 的鉴别性用例才红。⚠️ **未做,已记 OQ-31**:`.jsonl.zst` 压缩(上游 `local_thread_store_compression`,实测 0.153.4 上 `default_enabled: false`)会重命名并**删掉原 `.jsonl`**,届时白名单不匹配、且 `planner.ts:432` 会把它当 PULL_NEW 写回去,形成循环 |
| 74 | **machineId 只铸一次、永不重铸**:`detectIdentityDrift`/`rotateMachineId` 换成三值 `judgeIdentity`(`same` / `renamed` / `foreign-platform`);hostname 变化只更新 label 与指纹、**保留 id**;platform 变化仍 fail-closed;账本被判 `machine-id-mismatch` 时**出一条通知**而不是无声重来 | (a) 维持按 hostname 轮换 ／ (b) 改用硬件 UUID(`IOPlatformUUID` / `MachineGuid`) ／ (c) 把 homedir 也算进身份 ／ (d) 只铸一次 + 陌生账本出声 | **(a) 的代价算反了**:ADR-22 写「轮换零风险」,依据是 machineId 从不参与决策 —— 但**换值**本身有代价:`parseObservations`(`state-store.ts:189`)见到不同 id 就判整本 `unusable`,于是静默窗口状态、`remoteHadNonZeroSize`、以及 **`lastConvergedHash`/`convergedSize`** 一起没了,而最后那个正是 ADR-61 用来分辨「用户主动 rewind」与「本机落后」的基点。**hostname 根本不是身份**:它可被手改,且 macOS 未设 `HostName` 时会随网络在电脑名与 Bonjour 名之间跳 —— 2026-09-14 实测一台机器**一天轮换 9 次**,等于每次切网络都悄悄解除一道数据安全保护。**(b) 用户提过,五条理由否决,两条是决定性的**:①machineId 会被写进**共享目录**(`lastWriter` / `createdBy` / `detectedBy`),绑到硬件等于把一个稳定的硬件标识发布进别人也能读的文件夹;②macOS 与 Windows 上**读不到**它,只能起子进程(`ioreg` / `reg query`),而本产物至今 `child_process` 命中为 0,一个会执行命令的同步插件在社区清单审核里是另一码事。**而且它解决的不是我们的问题** —— machineId 本来就是铸出来的随机 UUID、本来就稳定,churn 来自**轮换触发条件**,不来自 id 本身。(c) 被既有设计明确拒绝过(「moved home 仍是同一台」),且有测试钉着。**(d) 选中**:重命名就是重命名。**那轮换原本要防的呢** —— 两台机器共用一个 id?查证后危害极小:锁在 `<stateRoot>/locks/` 是**本机的**,共享目录里 machineId 只出现在探测文件名与审计字段。真发生时账本会判 `machine-id-mismatch`,**那条路径现在会出声**,而不是像过去那样被 hostname 漂移淹没成常态。**门禁**:`artifact-smoke` 新增一条「产物不得出现 `child_process`/`spawn`」——把硬件 UUID 这个决定写死,免得日后被悄悄改掉。两条注入各自会红 |
| 75 | **append-jsonl 的 divergent 之前加一道语义等价判定**：两侧等长时逐行规范化比较（成员按键名排序、**标量按源文本原样照抄**），全等则判 `PULL_OVERWRITE` / `equivalent-serialisation`——**永远收敛到同步夹那一份**；同一份远端字节只收敛一次，再遇到判 `NOOP` / `equivalent-left-as-is`。判定函数 `compareRecordwise` 放在 `src/domain/equivalence.ts`（纯函数、无 Node），**开关放在引擎调用点**（`mode === "append-jsonl"` 且未超体积上限），`relate()` 保持只讲字节 | (a) 维持现状：divergent 一律 CONFLICT ／ (b) 判 `NOOP`、两边都不动 ／ (c) 按 hash 字典序选胜者 ／ (d) `JSON.parse` + `JSON.stringify` 做规范形式 ／ (e) 放进 `relate()` 里,对所有 provider 生效 | **(a) 是被实测推翻的那一条**：2026-09-14 普查同步夹里 54 个 codex 文件,**10 个正在转圈**——两台各自迁移同一条 legacy 会话,上游 `FileChangeItem.changes` 是 `HashMap<PathBuf, FileChange>`(`items.rs:401`)、Rust 每进程迭代顺序随机,于是「一次改多个文件」的轮次键序不同:**等长、等行、逐条记录相同、字节不同**。10/10 逐行核对语义差异为 0。两边用户各点「保留本机版本」,循环五天无终点。**(b) 表面安全,实际把病拖成慢性**:两份永不一致,每轮都要重算、报告每轮都有一行,而且 §7.2 的下游(收敛基点、ADR-61 的截短保护)永远拿不到基点。**(c) 是真正危险的那个**:约一半情况选中本机,于是要 push——两次写、两份备份,还要往第三方同步工具正在写的目录做原子写;更要命的是它**不是吸收态**,N 台机器可以来回选到任意一台,乒乓照旧。「只往本机写、绝不写进同步夹」才是终止性的来源:同步夹那一份是唯一吸收态,任何一台都无法给对端制造新变体,N 台各写一次本地即全体收敛。**(d) 会授权一次真正的数据覆盖**:`JSON.parse` 把 `{"n":12345678901234567890}` 和 `…891` 折成同一个 double,规范文本相同、长度也相同,于是两条**不同的记录**被判等价并互相覆盖——所以是手写扫描器、标量从源切片逐字复制;重复键直接判本行失败(排序后成员表相同,last-wins 又让结果取决于到达顺序),数组顺序、行边界移动、解析不了的行一律退回 divergent。行边界那条不只是洁癖:codex 的 paginated 历史按**字节偏移**建索引,等长排列天然保证「每条记录仍从原位开始」。**(e) 越界**:`relate()` 是 `PrefixRelation` 的唯一产地,把一个 provider 的上游缺陷塞进去,等于让所有 provider 都为它付代价(以及为它的 bug 担风险)。**churn stop 的必要性**:收敛之后本机与同步夹逐字相同,除非**本机**又被重写——CLI 每次启动重新序列化就是这个形状。届时远端字节仍是上次收敛的那一份,于是「远端 hash == 收敛基点 hash」这个已有状态正好是判据(EV-1 允许的方向:记忆只能把写变成不写)。代价是两份等价文件停在原地不再对齐字节——按定义不丢任何记录。**顺带修的**:`predict()`(恢复面板的「下次同步会怎样」)改用同一个 `compareRecordwise`,否则面板会对一条 7b 会静默解决的分歧说「会引发冲突」,用户为了一个永远不会出现的选择去恢复版本;`rotationDeferred` 的提示语原本写「这个 provider 重写整条记录」,ADR-75 之后 codex 也会触发它,于是改成只讲规则不点 provider。**注入验证**:10 条注入(调用点不问、7b 改成 push、churn stop 拿掉、引擎不报基点、改用 JSON round-trip、重复键 last-wins、不查等长、数组也排序、`predict()` 不问、面板文案借用邻居)各自会红;**唯一未能证明是承重的是那条 LF 早退**——它被下一行的「两侧行长必须相等」完全蕴含,注释已据实改写,不再声称它是第二道闸 |
| 76 | **dry-run 下 `loadOrCreateMachine` 不落盘**：`refresh(\{ persistIdentity \})` 由 `prepare(\{ dryRun \})` 传入，首次铸造与改名都只在内存生效，并把「少写了什么」折进本轮报告的 notices | (a) 维持现状(dry-run 也写 machine.json) ／ (b) 没有 machine.json 时 dry-run 直接拒跑(fail closed,ADR-21) ／ (c) 把 dryRun 提升为持久设置,让 `refresh` 自己读 | **(a) 违反 §7.6 的字面承诺**:状态目录是「保证不写入的五棵树」之一,而这是 `refresh()` 里仅剩的一处写。触发路径很窄但正是最该保护的那个——`prepare()` 只在 `!this.machine` 时才 `refresh()`,也就是**全新安装或身份文件丢失**时;而 dry-run 最常见的场景恰恰是「我刚配好 syncDir,还不确定填对没有」。**既有的 dry-run 测试全部先 `configure()`**,身份文件在快照之前就已存在,所以这处写入在快照里根本不可见——缺的不是断言而是**前置状态**,补测试时要先删掉 machine.json 再 `restart()` 一个新 runtime(内存里那份比文件活得久,而任何会重读的路径也都会重建)。**(b) 是过度的**:§10.3 说 machineId 从不参与决策,内存里给一个临时 id 不会改变本轮任何判定;而它确实会让 `parseObservations` 判 `machine-id-mismatch`、整本账本作废——但**真实 pass 在同样缺文件的情况下会一模一样地作废**(ADR-74),所以 dry-run 报出来的「全部首次见面」正是真实 pass 会做的事,报告仍然诚实。拒跑反而让用户在最需要预览的时刻看不到预览。**(c) 走不通**:`dryRun` 是逐次调用的参数不是设置(`syncNow({dryRun})`),提升成设置等于给它造一个新的持久状态,还要处理两者不一致。**不能沉默**:dry-run 悄悄少做一件事,报告就不再是完整交代——所以把它写进 notices,与 `pass-runner` 折叠 ledger notice 的做法同一条路子。五条注入各自会红 |
...
| **OQ-14** | **Grok 的 rewind 与 `/compact` 是否就地重写 `chat_history.jsonl`** | ✅ **已测(2026-08-29):是。查出的阻塞级缺陷已修(2026-08-30,ADR-61)** | 人工在沙箱 TUI 内补测(macOS,grok 1.0.5,4 张快照,[findings](./findings/2026-08-29-oq14-grok-rewind-compact.md))。**rewind 就地截断** `chat_history.jsonl`(165,566 → 159,924 B,69 → 53 行)与 `rewind_points.jsonl`(515 → 206 B);**`/compact` 整份重写**(161,841 → 21,699 B,59 → 5 行),被压掉的内容搬进新建的 `compaction/segment_000.md`,**与 Claude Code / Codex 的「追加进同一文件」(OQ-8 / OQ-13)不同**;会话 uuid 不变,不是 fork。`updates.jsonl` / `events.jsonl` **穿过 rewind 与 compact 全程严格追加**。⛔ **本行原先写的「风险已封顶 ⇒ 判 `CONFLICT`、不丢字节」是错的,已删**:那句话把「同机新旧两版」当成了比较对象,而插件比的是**本地 vs 同步目录**。rewind 之后本地(159,924)若是远端(165,566)的严格前缀,走的是 §7.2 #9 `r-extends-l` ⇒ **`PULL_OVERWRITE`**——**rewind 被静默撤销,且因通知门控在 `opaque-file` 而一句提示都没有**;同一次 pass 里 `summary.json` 反而把 rewind 后的状态推了出去,终局是两机都没有过的混合态。**已端到端复现(单机即可)**。字节不丢(覆盖前必备份,I1 成立)。**前提待证**:只证到前 131,072 字节相同,r01 尾部 28,852 B(18%)无覆盖,所以「纯截断 ⇒ PULL_OVERWRITE」与「尾部也重写 ⇒ CONFLICT」两支未分——补测协议见 `tmp/probe-m2/AGENTS.md` 6c.6。**已修(ADR-61)**:①跌破收敛基点 ⇒ §7.2 #9a 判 `CONFLICT`(信号挂在收敛基点 size 上——账本 `sig.size` 是死码,`size` 在 `signaturesEqual` 里,一变短就判不稳定、该轮 DEFER 并把账本刷新);②group 内有成员冲突时其余成员不许写(§7.1 P5),primary 因此不会把 rewind 后的记录推出去;③§9.1.6「M1 必做」的 `PULL_OVERWRITE` 提示扩到全部合并模式,点名会话与备份路径。**前提已证实(2026-08-30 二次补测)**:`prefix-check` 字节级判定 **rewind = 纯截断**(`chat_history.jsonl` 44,968 → 39,790 B、`rewind_points.jsonl` 515 → 206 B,新版均为旧版的严格前缀)⇒ **确实是 `PULL_OVERWRITE` 那一支**,不是 `CONFLICT`。`/compact` 相反(41,711 → 19,818 B,**NOT A PREFIX**)⇒ 本就落在封顶的 `CONFLICT` 上。危险窗口:rewind 后只要再发一条消息,`chat_history.jsonl` 就与旧版互不为前缀而退回 `CONFLICT`——**静默撤销只发生在「rewind 完、还没说话」这段时间里的同步**。常见用法(rewind 完立刻接着聊)只有几秒,pass 正好落进去的概率不高;但**为「删掉刚才那段」而 rewind 然后就关掉**的场景,这个窗口是无限长的——而那恰恰是 rewind 最要紧的用法。(残留未测:`rewind_points.jsonl` 过了窗口后是否仍是旧版前缀;两支都被 ADR-61 的「跌破收敛基点」判据覆盖,不阻塞)。✅ **修复已于 2026-09-01 真机复验**(验收步骤 C):截短 18,870 → 17,886 B ⇒ `CONFLICT` / `local-shrank-below-converged`,**同步后仍是 17,886**,两支都进隔离区([findings](./findings/2026-09-01-claudian-2.2.5-acceptance.md)) |
...
| **OQ-24** | **`claudian` provider 自身不认识 `devices/` 层** | 🟡 **可见性已由 ADR-69 解决;provider 的完整复制仍未做** | 原目标是让该 provider 完整复制 `.claudian/sessions`。调查后发现**那解决不了用户的问题**:设备 key 是机器本地的,`SessionStorage` 只读自己那个目录,所以把 `devices/**` 搬到对端**对对端完全惰性**——保真有价值,功能价值为零。真正要解决的「同步过来的会话在对端看得见」由 **ADR-69** 用 flat 层解决(**搬运**,不是 ADR-67 那版已被推翻的复制),且**与该 provider 是否启用无关**(用户的 vault 走 iCloud 时同样生效)。provider 的完整复制仍待做,难点未变:`logicalId` 是备份路径的单个路径段,而全长 device key 会让 Windows 路径撞 260 上限,ADR-54 又不许概率性短前缀 |
| **OQ-25** | **`.assigned.json` 要不要同步** | ✅ **已关闭(2026-09-01):不同步、也不写** | Claudian 2.2.5 新增的第四种记录(只在顶层)。它声明「本 conv 归属设备 X」——**同步过去会让对端的 Claudian 把这条会话藏起来**,与用户启用同步的意图相反;不同步则对端落回 device/unscoped/legacy 阶梯、会话可见。**决定已定档在 ADR-69**:它的检查排在 `selectSessionMetadataCandidate` 里任何 metadata 路径之前(`SessionStorage.ts:71`),**复制一份过去正好把这条会话在对端彻底黑掉**——与用户启用同步的意图正相反。不同步则对端落回 device/unscoped/legacy 阶梯、会话可见。2026-09-01 验收步骤 D6 复核:共享后对端那一行确实出现 `Assign to this device` 按钮(代价如实写在设置文案里),**未点击时 vault 内不产生任何 `.assigned.json`** |
| **OQ-26** | **「看得见 store 却一条都没准入」需要一条告警** | ✅ **已关闭(2026-09-01,ADR-66)** | 通道用 `healthCheck().warnings`——它在 P1 每个 provider 跑一次、在任何 group 之前,是零 group 时唯一幸存的出口(ADR-62 的 `unprovenOmissions` 挂在 group 上,恰好此时没有载体)。信号取「store 里有本版本不下探的目录」,正常恒为空。✅ **2026-09-01 真机复验通过**(验收步骤 B):通知逐字出现并点名探针目录,`rmdir` 后消失 |
| **OQ-27** | **上游每个 Obsidian 会话只推导一次「往哪层写」** | ✅ **已关闭(2026-09-04,ADR-71)** | `ConversationRepository.metadataTargets` 由 layout-ready 后的单次延迟扫描填好,之后既无 watcher 也不重读已缓存的会话。**这不是竞态,是结构性陈旧**:任何在同进程里搬动记录的操作都必然晚于那次推导。⛔ **本行原先写的「上游侧的根治是让 `saveMetadata` 写回 `loadOnce()` 已算出的 `source`,值得给上游提」是错的,已删**,两处都错:①**这是上游写明的不变式,不是缺陷**——`src/core/AGENTS.md:23` 原文「Readers plus metadata and input-ledger writers must treat that fence as authoritative **even when stale unscoped metadata remains**」,并有四条同名测试(`ConversationPersistence.test.ts:200/698/739/761`)。正确性不在那张内存表上,而在磁盘上的 `<id>.assigned.json` 围栏,`assertMetadataWriteAuthority` **每次写入都重读它**,陈旧路径一律 fail-closed(抛错或空转),不会写错层。②`source` 也不是被丢掉的,是被 `adoptMetadataConversations` **缓存**了(`:266-269`),`writeMetadata` 每次都重放(`:1916`);把它穿进 `saveMetadata` 什么也不改变,除非在写入时重新读盘——那既昂贵又不够(争用窗口里 peer 重读得到的是 null,根本没有「读到的那一层」可写回)。**插件侧不依赖它**:ADR-71 依赖的是这个行为**存在**,而不是它是个 bug |
| **OQ-28** | **搬运后删除会话会复活** | ✅ **已关闭(2026-09-04,ADR-71)**;**该缺陷随 0.3.0 出厂** | 陈旧目标让 `markDeleted` 把墓碑写进 device 层,flat 那份无墓碑,下次扫描取舍阶梯落到 flat ⇒ 会话回来。窗口是「搬运之后到下次 Obsidian 重启之间」,通常是一整天。修法:flat 仍等于本机发布的基点时,把墓碑补写进 flat |
| **OQ-29** | **上游的「Assign to this device」把围栏与搬移分成两个文件,同步中途可被另一台机器写回** | ⏳ **已定位,未在真机复现;是否上报待定** | 与本插件**无关**,纯上游两机 + 任意 vault 同步即可达。`assignMetadataToDevice` 先写 `<id>.assigned.json`(`ConversationPersistenceStore.ts:145`)再 rename 记录进 device 层(`:150`),**两个文件之间没有跨文件的同步顺序保证**。若对端在「rename 已到、围栏未到」的窗口里写入,`assertMetadataWriteAuthority` 在 `:218` 见 `status === 'missing'` 直接放行 ⇒ 对端**把 A 刚搬走的 flat 记录又建了出来**。那份此后无人读:A 侧 device 优先(`SessionStorage.ts:73-75`)、对端重启后因外来围栏返回 null(`:71`)⇒ **对端的标题/sessionId/usage 静默丢失**。丢的是元数据不是对话正文(正文在 provider 自己的文件里)。**未上报的理由**:窗口宽度未测,我们没有真机复现,而复现步骤里那一步只能用手工删文件模拟同步延迟——那很容易被合理地判成「你在插件背后改文件」。真撞上再报,届时带证据 |
| **OQ-30** | **设置面板是否改用 1.13.0 的声明式 API(`getSettingDefinitions`)** | ⏳ **已评估,0.3.2 刻意未做** | 社区清单的自动审核提了两条(Warning + Recommendation,**不是**导致 Failed 的那两条 Error):不实现它则设置项**不出现在 Obsidian 1.13.0+ 的设置搜索里**,且 `display()` 自 1.13.0 起标记弃用。**代价被 typings 写死了**:`obsidian.d.ts:6633` 原文「Not called when `getSettingDefinitions` returns a non-empty array」—— 即**不能增量采用**,一旦返回非空数组,1.13+ 用户看到的整个面板就只由声明式定义渲染,`display()` 只剩作为 <1.13 的回退。而本面板是动态的:状态区按 `phase` 换按钮、provider 列表由注册表循环生成、首次启用有 dry-run 闸门、冲突计数实时变。**要么整体重写、要么两套并存长期同步**,任一都不是「清掉一条 lint」的体量,且直接压在 25 条 UI 测试上。另有 `minAppVersion` 取舍:现为 1.5.0,若放弃回退则要抬到 1.13.0。**弃用不等于移除**,`display()` 目前照常工作,所以这条不阻塞;要做就单独一版做,别搭在发布车上 |
| **OQ-31** | **Codex 的 `.jsonl.zst` 压缩落地后 codex 会停止同步,并可能形成复活循环** | ⏳ 已定位,未生效 | 上游后台 worker 压缩「冷」rollout 并 `remove_file` 删掉原 `.jsonl`(`rollout/src/compression.rs`)。实测 0.153.4 上 `local_thread_store_compression` 与 `background_paginated_rollout_migration` **都是 `default_enabled: false`(under development)**,所以今天没生效。一旦默认打开:①白名单 `*.jsonl` 不匹配 `.zst` ⇒ 沉默停同步;②本地文件消失后 `planner.ts:432` 判 `PULL_NEW` 把 `.jsonl` **写回去**,再被压缩删掉 —— 循环。**不要把 `.zst` 加进白名单**:zstd 帧没有前缀关系,压缩是「写新+删旧」的替换,而且两台各自压缩产出不同字节 |
| **OQ-32** | **没有一条「这个版本不认识的 codex 文件」的普查告警** | ✅ **已关闭(2026-09-10,ADR-73)** | ADR-73 修了已知的一种改名,但下一次上游改名仍会沉默。应在 `healthCheck` 里做一次**只看名字**的 `YYYY/MM/DD` 普查,数出 `rolloutIds()` 拒绝的文件数并点名一个例子 —— 与 ADR-66 对 `devices/` 做的是同一件事。**普查而非枚举**:它对 `_<rolloutId>`、`.zst` 和还没见过的形态一律有效。**已实现**:`healthCheck` 里一次只看名字的 `YYYY/MM/DD` 遍历(不 stat、不读、不过 vault scope —— 解析不了的文件根本没有 id 可以拿去比对 scope,过一遍等于把要报的东西筛掉),数出 `rolloutIds()` 拒绝的名字并点名两个。**已知属于别人的不算**:同步工具的冲突副本(按目录判,因为副本只有挨着原件才认得出)与本插件自己的 `.aiss-tmp-*`,否则告警长亮、没人再读。三条注入各自会红 |
| **OQ-33** | **同步报告的 Files 表无序,上百行里找不到该找的那一行** | ⏳ 未做 | `sync-engine.ts:909` 原样传出 `actions`,`report-modal.ts` 按目录遍历顺序渲染,**从不排序**。2026-09-10 验收人按「Session 列升序」去找,看到 `01a08142` 后面直接是 `04995d0e` 就判定那一行不存在 —— 而那个顺序根本不存在。这是第二次有人在报告里找不到该找的行(上一次是被自动 pass 冲掉)。**不是正确性问题**(每条路径都会 push 行、表格也不截断),是可用性:按 Session 排序,或给一个过滤框 |
| **OQ-21** | **`recordLedger` 的「未变」判断与 `signaturesEqual` 不一致** | 🟡 **已消除不一致(2026-08-30),但未找到可达后果** | 它只比 `size`/`tailHash`/`mtimeMs` 三项,而 `judgeStability` 比五项(多 `ctimeMs`/`ino`);且自行重算 `firstSeenMs` 而不用裁定结果,所以 §9.1.2 的时钟回拨修正(注释写着「bounded wait, not a permanent defer」)**从来没被写回过**。**已改**:统一用 `signaturesEqual`,并把同一个未来时间戳钳制放到写入处。**但要诚实**:两半都构造不出可达的后果。五取三只在「内容、mtime 全同,仅 ctime/ino 变」时才与五取五分歧——而同步工具的 rename 几乎总会改 mtime(实测:`utimes` 恢复 mtime 都存在精度损失,连人为构造都做不干净);时钟回拨那一半则在集成路径上被 readiness 先拦住(实测四次 pass 全是 `remote-not-ready`)。而且默认 `localQuietMs=3000` 远小于 5 分钟的 pass 间隔,窗口本就形同虚设。**所以这是「消除一个模块里两套『同一个文件』的定义」,不是修一个已知缺陷**——它的实际咬合力要等 OQ-16 把窗口调到有意义的值才出现 |
| **OQ-22** | **§9.1.3 的 `PULL_NEW` 快速通道是死的** | ✅ **已关闭(2026-09-01,ADR-70):删掉** | `sync-engine.ts` 里 `pullNewFastPath: false` 是硬编码的,`allowsPullNewFastPath()` 在生产代码里零调用者(只有测试在调)。所以「对端新会话不必等满静默窗口就落地」这条承诺目前不存在。**按 ADR-63 的先例废除并改了规范**:它的第三个条件就是 ADR-63 刚废掉的轮内第二次观察,而剩下的 `fullyParsed` 对 append-only 文件结构性失明(半落地的前缀是合法 JSONL)。同上,附带发现 |
...
| **OQ-16** | **稳定窗口该取多大** | ✅ **已关闭(2026-08-30,ADR-64):3000 → 15000** | 无需新实验——把 OQ-17 的两轮 100 ms 采样重读即可([findings](./findings/2026-08-30-oq16-quiet-window.md))。合并全部非 lock 文件的变化时刻(= group 见证的合成签名所见),最长整组静止 **3,344 / 8,212 ms**,出厂的 3000 ms 在一次 turn 里被骗 1–2 次;15,000 ms 两轮清零。`events.jsonl` 这个全场最忙的见证文件本身就停了 8.2 秒,说明 ADR-58 修对了粒度但没定值。代价近零(5 分钟间隔下窗口从不生效)。`remoteQuietMs` 未动 |
