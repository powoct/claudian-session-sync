...
| **OQ-24** | **`claudian` provider 自身不认识 `devices/` 层** | ⛔ 已确认,待设计(默认关闭,当前安全) | 需要真正的设计决定:该 provider 的 `logicalId` 是**按文件**的(`conv-x.meta` 与 `conv-x.inputs` 各自成组,ADR-48),而同一个 conv **可以同时存在于顶层与设备层**(上游候选选择器整段在处理这情况)⇒ 两者 `logicalId` 相同 ⇒ 引擎 `byLogicalId` 静默覆盖其一;把层编进 `logicalId` 也不行——它是**备份路径的一个路径段**(ADR-54),带斜杠会造嵌套备份目录。**当前状态安全**:`listSessions` 看不见设备层 ⇒ 不同步 ⇒ `targetPathFor` 不会把它们拍平到顶层。危险的是「只修 listSessions 不修 targetPathFor」 |
