...
| **OQ-32** | **没有一条「这个版本不认识的 codex 文件」的普查告警** | ⏳ 未做 | ADR-73 修了已知的一种改名,但下一次上游改名仍会沉默。应在 `healthCheck` 里做一次**只看名字**的 `YYYY/MM/DD` 普查,数出 `rolloutIds()` 拒绝的文件数并点名一个例子 —— 与 ADR-66 对 `devices/` 做的是同一件事。**普查而非枚举**:它对 `_<rolloutId>`、`.zst` 和还没见过的形态一律有效 |
