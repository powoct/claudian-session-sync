...
| **OQ-24** | **`claudian` provider 自身不认识 `devices/` 层** | 🟡 **可见性已由 ADR-67 解决;provider 的完整复制仍未做** | 原目标是让该 provider 完整复制 `.claudian/sessions`。调查后发现**那解决不了用户的问题**:设备 key 是机器本地的,`SessionStorage` 只读自己那个目录,所以把 `devices/**` 搬到对端**对对端完全惰性**——保真有价值,功能价值为零。真正要解决的「同步过来的会话在对端看得见」由 **ADR-67** 用 flat 层解决,且**与该 provider 是否启用无关**(用户的 vault 走 iCloud 时同样生效)。provider 的完整复制仍待做,难点未变:`logicalId` 是备份路径的单个路径段,而全长 device key 会让 Windows 路径撞 260 上限,ADR-54 又不许概率性短前缀 |
