...
| **OQ-25** | **`.assigned.json` 要不要同步** | ⛔ 待想清产品语义 | Claudian 2.2.5 新增的第四种记录(只在顶层)。它声明「本 conv 归属设备 X」——**同步过去会让对端的 Claudian 把这条会话藏起来**,与用户启用同步的意图相反;不同步则对端落回 device/unscoped/legacy 阶梯、会话可见。当前不认它(fail-closed),但这是个没想清的决定而非疏忽 |
| **OQ-26** | **「看得见 store 却一条都没准入」需要一条告警** | ✅ **已关闭(2026-09-01,ADR-66)** | 通道用 `healthCheck().warnings`——它在 P1 每个 provider 跑一次、在任何 group 之前,是零 group 时唯一幸存的出口(ADR-62 的 `unprovenOmissions` 挂在 group 上,恰好此时没有载体)。信号取「store 里有本版本不下探的目录」,正常恒为空 |
