...
| **OQ-27** | **上游每个 Obsidian 会话只推导一次「往哪层写」** | ✅ **已关闭(2026-09-04,ADR-71)** | `ConversationRepository.metadataTargets` 由 layout-ready 后的单次延迟扫描填好,之后既无 watcher 也不重读已缓存的会话。**这不是竞态,是结构性陈旧**:任何在同进程里搬动记录的操作都必然晚于那次推导。上游侧的根治是让 `saveMetadata` 写回 `loadOnce()` 已经算出的 `source`,值得给上游提;插件侧不依赖它 |
