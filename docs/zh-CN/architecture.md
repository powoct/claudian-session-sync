...
| **OQ-26** | **「看得见 store 却一条都没准入」需要一条告警** | ⛔ 已确认,待做 | 2026-09-01 这次最贵的不是缺陷本身而是**它一声不响**。难点是通道:ADR-62 的 `unprovenOmissions` 挂在 `SessionGroup` 上,而候选集为空时恰好没有载体——正是该失效形态。要给 `PassReport` 加独立字段并从 adapter 接上去 |
