...
以及单条发布会带着整轮对账跑(点一条 = 动全部)。

修完补跑了**附录 F**(同日,10 分钟,单机):开关 ON 造出带基点的 X → 加 marker 使其分叉
→ **关掉开关**再造旁观者 Y,同一条链验三处。**F1/F2/F3 全过**。
判据是**绝对断言**而非新旧对比:F1 要求「Y 在哪层 / flat 条数 / marker 计数」三值逐一不变
(账本 mtime 停在开屏之前是最有力的旁证),F2 要求 Y 的字节、sha、mtime 全部不变。
该轮又抓出三条并已修:`inspectOnly` 改为**必填**(忘记 = 编译错误,而不是加开关判断——
那会破坏「关掉开关后仍能修历史残留」)、`rootOverride` 可被清空、发布结果改为三态。

### 9.10 Codex 的新格式(ADR-73,**0.3.3 的闸门**,单机)

codex 在 `rollout_id != thread_id` 时把 rollout 命名成
`rollout-<ts>-<threadId>_<rolloutId>.jsonl`,而 **revert 恰好产生这种情况**。旧白名单不认识它
⇒ 一条 rewind 过的会话**最新的历史从来没同步过**,而 rewind 前那份照常同步、报告照常
「up to date」。

| 步 | 断言 |
|---|---|
| **G2 ⭐** | 带 `_` 的文件**进了同步夹**,sha256 与本机一致;rewind 前那份也还在 |
| **G3 ⭐** | 手工放一个不认识的名字 ⇒ Notices **点名它**并说「not travelling」;删掉后消失 |

⚠️ **会测的会话必须在 Claudian 里建** —— 准入读的是 Claudian 的对话记录,`codex exec` 起的会话
没有记录,G2 会一个文件都验不到,**而那是正确行为**。

⚠️ **回退只能从 TUI(Esc-Esc)或 app-server 的 `thread/revert` 触发**,CLI 没有子命令;
Claudian 开着该会话时会被拒。

✅ **2026-09-10 G1–G4 通过**(单机 macOS),
[findings](./findings/2026-09-10-codex-rollout-rename-acceptance.md)。
该轮的「报告里没有那一行」经查**不是缺陷**:Files 表不排序,按字母序找不到 —— 已记 OQ-33。

### 9.11 键序变体不再是冲突(ADR-75,**0.3.4 的闸门**,两机)

codex 的 `FileChangeItem.changes` 是 `HashMap<PathBuf, FileChange>`,Rust 的 HashMap 每进程
迭代顺序随机。两台各自迁移同一条 legacy 会话 ⇒ **等长、等行、逐条记录相同、字节不同**。
0.3.4 之前一律判 `CONFLICT`,而两边各点「保留本机版本」会让对方再次不一致 ——
实测转了五天(2026-09-14,同步夹 54 个 codex 文件里 10 个在转圈)。

| 步 | 断言 |
|---|---|
| **H2 ⭐** | 键序变体判 `PULL_OVERWRITE` / `equivalent-serialisation`,不进隔离区,不弹 history-replaced |
| **H3 ⭐⭐** | **同步夹那一份字节未变**;来回两轮后四份副本全等,且无任何写入动作 |
| **H5 ⭐** | 本机再次被重新序列化 ⇒ `NOOP` / `equivalent-left-as-is`,文件不被改回去,备份不新增 |
| **H6 ⭐** | 内容真正不同 ⇒ 仍然 `CONFLICT`,两份都在隔离区 |
| **H7** | dry run 不创建 `machine.json`,且 Notices 说明少写了什么(ADR-76) |

**H3 是核心。** 「永不写进同步夹」是乒乓终止的全部理由:同步夹是唯一吸收态,
N 台机器各写一次本地即全体收敛。H3 失败(本机把自己那份推上去)意味着乒乓只是换了形状。

⚠️ **样本来源**:上游的随机键序无法按需复现,所以套件用 `scripts/permute-keys.mjs` 造一个,
它**自证等长**(先要求每行 `stringify(parse(line)) === line`,再要求排列后逐行长度不变,
任一不满足就拒绝写出)。**若 2026-09-14 那 10 条真实样本尚未手工解决,应优先用它们**。

⚠️ **会测的会话必须在 Claudian 里建**(同 §9.10)。

⚠️ **尚未执行。0.3.4 是本项目第一个没有真机验收轮就发布的版本** ——
2026-09-15 发布时两台机器都在跑别的任务,取舍已记入 HANDOFF 发布记录表。
剧本写在验收套件的「附录 · 键序变体不再是冲突」,**下一轮与当时那一版的闸门一起跑**。
自动化背书:1083 条用例(含两机 L2 世界的多轮定点测试)、三平台 CI、17 条注入里 16 条验红。
...
| **Codex 新格式** | §9.10 G1–G4 ✅（2026-09-10）——0.3.3 的发布闸门 |
