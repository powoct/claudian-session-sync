...
> 更新时间：**2026-09-15**（**0.3.4 已发布**；M1/M2/M4 完成；M3 主体交付；Claudian 2.2.5 兼容性已通过两机验收）。本文描述**当前进度快照**，供下一个会话（或下一个人）接手。
...
### 2026-09-04：搬完的记录留不住，以及「看一眼就等于搬了」

ADR-69 假定搬进 flat 层的记录会留在那儿。**不会** —— 上游每个 Obsidian 会话只推导一次
「这条会话往哪层写」，搬运发生在那之后，所以它会按陈旧判断在设备层重新写一份，
而它读取时设备层优先 ⇒ **共享那份从此冻结**。报告机器上 6 条，最严重的一条对端落后三天。
0.3.1 把搬运从一次性迁移改成**每轮对账**（ADR-71），并修掉同源的**删除复活**缺陷
（那条随 0.3.0 出厂）。

**两轮单机验收，各抓出缺陷各修一次**，这一点值得记住：

- 第一轮（E0–E4）三个闸门全过，但验收人**读代码**发现修复屏的读取路径**会写盘且不看开关**
  —— 开关关着的用户只要打开一次那个屏幕，本机记录就全被搬进 flat。**看一眼不等于同意。**
- 第二轮（附录 F，10 分钟）三个闸门全过，又发现**忘记传 `inspectOnly` 就会重开那个洞**
  —— 改成必填，忘记 = 编译错误。同轮还修了「清空 storage folder 不生效」
  与「Nothing was published 同时表示两件事」。

**教训**：这个功能写进的是**另一个程序拥有的目录**，而那个程序的行为不是我们能约定的。
两轮验收抓到的都不是「算错了」，而是**「在不该写的时候写了」**。

### 2026-09-10：codex 改了文件名，我们沉默丢了一条会话的历史

codex 在 `rollout_id != thread_id` 时把 rollout 改名带 `_`，而 **revert 恰好产生这种情况**
（它不改写原文件，而是新建一个、旧的原样留着，只挪 SQLite 指针）。旧白名单不认识那个名字
⇒ 一条 rewind 过的会话**最新的历史从来没同步过**，而 rewind 前那份照常同步、报告照常
「up to date」。**与 2026-09-01 的 `devices/` 是同一种沉默。**

**所以这一版真正的收获不是那个修复，是 OQ-32 的普查告警**：`healthCheck` 现在只看名字
数出「这个版本不认识的 codex 文件」并点名。它**不问为什么不认识**，所以对还没见过的形态
同样有效 —— 上游下次改名，会是一句话，而不是又一次沉默。

**教训**：三次沉默（`devices/`、看一眼就等于搬、这次改名）都不是算错，都是
**「该说话的时候没说话」**。写在别的程序拥有的目录上，检测缺席比检测错误更重要。

**没有待办的人类动作。** 下一步做什么见下面的「下一步」表与「验收欠账」，都不紧急。

### 2026-09-15：两台机器写出同一段对话，却永远谈不拢

codex 的 `FileChangeItem.changes` 是 `HashMap<PathBuf, FileChange>`，Rust 每进程迭代顺序随机。
两台各自迁移同一条 legacy 会话 ⇒ **等长、等行、逐条记录相同、字节不同**。插件只比字节，
判 divergent ⇒ 隔离；两边用户各点「保留本机版本」⇒ **对方又不一致**。实测 2026-09-14：
同步夹 54 个 codex 文件里 **10 个在转圈，转了五天**。

修法的关键不是「认出等价」，是**收敛到哪一份**：规则 7b **永远采纳同步夹那一份、绝不写进同步夹**。
一条永不写同步夹的规则不可能给对端制造新变体 ⇒ 同步夹是唯一吸收态 ⇒ N 台各写一次本地即全体收敛。
按 hash 选胜者会有一半情况要 push，**乒乓照旧**。

**教训（与前三次沉默不同的一种）**：这次插件不是没说话，是**说得太肯定**——
它把「字节不同」直接读成「内容分歧」，而那一步推理从来没人写下来过。
`divergent` 这个名字本身就藏着这个假设。

**另一条**：ADR-74（machineId 只铸一次）暴露出 dry-run 一直在写自己的身份文件。
既有的 dry-run 测试全部先 `configure()`，身份文件在快照之前就已存在，**所以缺的不是断言而是前置状态**。
写「保证不写入」这类测试时，要先问「这个写入在我的起始状态里可见吗」。

**有一件待办的人类动作**：0.3.4 **没有真机验收轮**（发布时两台机器都在跑别的任务）。
剧本已经写好并标注「尚未执行」——[testing §9.11](docs/zh-CN/testing.md) 是断言表，
验收套件的「附录 · 键序变体不再是冲突」是操作步骤（H0–H8，两机约 40 分钟）。
**下一轮验收请把它和当时那一版的闸门一起跑。** 套件里新增了 `scripts/permute-keys.mjs`：
上游的随机键序无法按需复现，它造一个并**自证等长**（不等长就拒绝写出）。
⭐ 若 2026-09-14 那 10 条转圈的会话**还没手工解决，就别解决** —— 它们是比造出来的更好的样本，
正好可以直接验「0.3.4 会不会自己收敛」。
...
| **0.3.4** | 2026-09-15 | **ADR-75 语义等价**（等长且逐行规范化后相同 ⇒ 采纳同步夹那份，不再判冲突；同一份远端字节只收敛一次）＋ **ADR-74 machineId 只铸一次**（不再按 hostname 轮换，账本与 ADR-61 的收敛基点不再被清空）＋ **ADR-76 dry-run 不再写身份文件**（§7.6 的第五棵树） | ⚠️ **没有真机验收轮**。凭据是 1083 条自动化用例（含两机 L2 世界里的多轮定点测试）＋ ubuntu/macOS/Windows 三平台 CI ＋ 17 条注入里 16 条验红（第 17 条是 ADR-75 里那条被下一行完全蕴含的 LF 早退，已据实记在 ADR 里而非当作承重）。**未在真机上验证过的**：codex 真实键序变体的收敛、10 条转圈会话解开后不再复发 |
| **0.3.3** | 2026-09-10 | **Codex 新格式**（ADR-73：revert 后的 `_<rolloutId>` 文件名；线程 id 准入、rollout id 标识）＋ **OQ-32 普查告警**＋ 备份失败不再掀掉整轮 pass | **§9.10 G1–G4 ✅**（2026-09-10，单机） |
| **0.3.2** | 2026-09-05 | 社区清单自动审核整改（`authorUrl`、构建来源证明、`electron` 字面量导入、popout 计时器） | 清单审核复跑 |
| **0.3.1** | 2026-09-04 | **ADR-71 搬运对账**（FOLD / RETIRE / 墓碑结转）＋ 修复屏；OQ-22 删掉死的快速通道；顺带修掉三处安全欠账（共享步骤在锁外、设备目录只查叶子、store 读不到当空目录） | **§9.9 E1–E3 ✅ + 附录 F 三闸门 ✅**（2026-09-04，单机两轮）——两轮各抓出缺陷并修，详见 findings |
...
| 1 | ~~OQ-19~~ ✅ **已修(ADR-59)**:binding 记 `vaultPath`,按当前打开的 vault 选。护栏更准而不是更松——身份被替换仍 fail closed(有测试);旧 binding 首次打开自己的 vault 时补盖,补盖前保持旧行为 | — |
| 2 | ~~M3 剩余:孤立 aux 清理命令~~ ✅ **已交付(ADR-60)**:命令「Clean up half-copied sessions」,四条判据(provider 认得 / 缺 primary / sync 目录也没有 / 安静 10 分钟),删除前先备份,永不自动 | **M3 全部交付完毕** |
...
| **OQ-30** 设置面板改用 1.13.0 声明式 API | ⏳ 已评估，刻意未做 | 不做则设置项不进 Obsidian 1.13+ 的设置搜索，且 `display()` 已标弃用（**未移除**，仍正常工作）。**不能增量采用**：`getSettingDefinitions()` 返回非空数组时 `display()` 根本不会被调用，所以要么整体重写这个动态面板、要么两套并存。单独一版做，别搭发布车 |
| **OQ-31** codex 的 `.jsonl.zst` 压缩 | ⏳ 已定位，未生效 | 实测 0.153.4 上 `local_thread_store_compression` 与 `background_paginated_rollout_migration` **都是 `default_enabled: false`**。落地后白名单不匹配、且 `planner.ts:432` 会把删掉的 `.jsonl` 当 PULL_NEW 写回去形成循环。**别急着写** —— 上游还在开发中，现在写等于按猜测编码；风险由 OQ-32 的普查兜住 |
| **OQ-33** 同步报告的 Files 表无序 | ⏳ 未做 | 上百行不排序，两轮验收都有人找不到该找的行。不是正确性问题，是可用性 |
| **OQ-29** 上游 assign 的围栏与搬移是两个文件，同步中途可被对端写回 | ⏳ 已定位，未真机复现 | 与本插件无关，纯上游两机即可达；丢的是元数据不是正文。未上报的理由见架构表 OQ-29 |
| **E4「一次面对多条」残留修复** | 未实测 | 两轮验收时机器上都没有残留可修（用户已手工对齐）。代码路径与 E3/F2 点的是同一个按钮 |
